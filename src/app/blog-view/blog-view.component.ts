...
export class BlogViewComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getCurrentBlogInformation(currentBlogId);
    // console.log(this.currentBlog.blogId);

    (($) => {
      $('.parallax-window').parallax({imageSrc: '../../assets/images/' + this.currentBlog.blogImage});
      $('.parallax-window').parallax({dataImageSrc: '../../assets/images/' + this.currentBlog.blogImage});
      $(window).scrollTop(0);
    })(jQuery);
  }

  getCurrentBlogInformation(blogId) {
    for (const blog of this.allBlogs) {
      if (blog.shortLink === blogId) {
